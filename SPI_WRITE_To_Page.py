"""Program 8-5  Write to a page of 25AA010A EEPROM through SPI
...
class SpiEEPROM1k:
    ...
    def __init__(self, id_, pins, params):
        ...
        self.spi = SPI(id_, sck = Pin(pins[0]), 
                    mosi = Pin(pins[1]), miso = Pin(pins[2]), baudrate = params[0], bits = params[1], polarity = params[2], 
                    phase = params[3], firstbit = params[4])
...
def dump_memory():
    ...
    for addr in range(0x00, 0x80, 0x10):     # 8 lines of 16 bytes per line
